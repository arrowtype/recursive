...
frames = 192 # currently must be in units of 100
...
def getWeightValue(t, curviness, axMin, axMax, loop="loop"):
...
for frame in range(frames):
    ...
    slntVal = getCurveValue(t, 0, 0, -15)
    wghtVal = getCurveValue(t, 0.7, 300, 900)
    xprnVal = getCurveValue(t, 0.5, 0, 1)
    ...
    if debug:
        ...
        x = getCurveValue(t, 0.8, 0, W, loop="nope")[0]
        size = padding * 0.15

        oval(x-size/2, (padding/2)-(size/2), size, padding)
