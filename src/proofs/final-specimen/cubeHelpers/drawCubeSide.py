...
def drawFlatCubeSide(axes=axes, xVar="wght", yVar="slnt", Xasc=True, Yasc=True, letter="a", rows=6, cols=6, MONOVal=0, CASLVal=0, wghtVal=300, slntVal=0, italVal=0.5, fileTag=""):
	...
	if save:
		...
		cubeExport = f"{outputDir}/cube--{now}-MONO_{MONOVal}"

		if not os.path.exists(f"{currentDir}/{cubeExport}"):
			os.makedirs(f"{currentDir}/{cubeExport}")

		path = f"{currentDir}/{cubeExport}/{docTitle}-{now}.{fileFormat}"

		if fileTag != "":
			path = f"{currentDir}/{cubeExport}/{fileTag}--{docTitle}-{now}.{fileFormat}"
