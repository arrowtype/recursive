...
def splitFont(fontPath, outputDirectory="fonts/rec_mono-for-code", newName="Rec Mono"):
	...
	for instance in instanceValues:

		print(instance)

		instanceFont = instancer.instantiateVariableFont(
					varfont, {"wght": instanceValues[instance]['wght'], "CASL": instanceValues[instance]['CASL'], "MONO": instanceValues[instance]['MONO'], "slnt": instanceValues[instance]['slnt']}
				)
		...
		newStyleLinkingName = instanceValues[instance]['style']
		...
		newFamName = currentFamName.replace(' Sans','').replace(oldName, newName).replace('Linear Light',instance.replace(" " + instanceValues[instance]['style'],''))
		...
		newFileName = fontFileName.replace(oldName,newName.replace(' ','')).replace('_VF','-'+instance.replace(' ',''))
		outputSubDir = f"{outputDirectory}/{instance.replace(' ','')}"
