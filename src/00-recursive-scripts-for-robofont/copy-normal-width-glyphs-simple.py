...
glyphsToNotMove = typicallyNarrowGlyphs + narrowPunctuation + sansSpecificForm + somewhatNarrowGlyphs + somewhatWideGlyphs + typicallyWideGlyphs
...
def setMarkColor():
    userColor = AskString("Mark copied glyphs as r, g, or b?", "g")
    ...
    if userColor == "r":
        ...
        markColor = (1*random.random(), 1*random.random(), 1*random.random(), 0.5)
...
def getMasterToCopyFrom():
    ...
    masterToCopyFromPath =  getFile("Please pick a master to copy normal-width characters from")
...
def getMasterToSendTo():
    ...
    masterToSendToPath =  getFile("Please pick a master to overwrite copied normal-width characters into")
    # print(masterToSendToPath)
    masterToSendTo = OpenFont(masterToSendToPath)[0]
    
    masterToSendToName = masterToSendTo.info.familyName + " " + masterToSendTo.info.styleName
...
def checkIfOkay(glyphsToOverwrite, masterToSendToName):
    proceedWithCopy = AskYesNoCancel("This will overwrite glyphs " + str(glyphsToOverwrite) + " in " + masterToSendToName+"." +" Proceed?")
